...
import { apiInstance } from './apiWithFallback';

// Use the fallback-enabled API instance if fallback is enabled
const shouldUseFallback = process.env.REACT_APP_ENABLE_API_FALLBACK === 'true';

// Determine the base URL based on environment (legacy function for non-fallback mode)
const getBaseUrl = () => {
...
const api = shouldUseFallback ? apiInstance : axios.create({
  baseURL: getBaseUrl(),
  headers: {
...
// Add interceptors only if not using fallback (fallback API has its own interceptors)
...
api.interceptors.response.use(
  ...
  // For fallback API, we need to add auth interceptors to the existing instance
  api.interceptors.request.use(
    config => {
      // Enhanced request logging
      debugLog(`🚀 REQUEST: ${config.method.toUpperCase()} ${config.url}`, {
  ...
  // Add auth response interceptor for fallback API
  api.interceptors.response.use(
    response => {
      // Enhanced response logging
      debugLog(`✅ RESPONSE: ${response.status} from ${response.config.url}`, {
    ...
    error => {
      ...
      // Handle forceLogout from backend
      if (error.response?.status === 401 && error.response?.data?.forceLogout) {
        console.log('🔄 Force logout detected - clearing auth data');
...
if (process.env.NODE_ENV === 'development') {
  ...
  // Add fallback debug functions if fallback is enabled
  if (shouldUseFallback) {
    const { checkApiEndpoints, getFallbackConfig } = require('./apiWithFallback');
    
    window.debugAPI.checkEndpoints = checkApiEndpoints;
    window.debugAPI.getFallbackConfig = getFallbackConfig;
  }
  ...
  debugLog('🔧 DEBUG MODE ENABLED', {
    ...
    availableFunctions: [
      ...
      ...(shouldUseFallback ? [
        'window.debugAPI.checkEndpoints() - Check all API endpoints',
        'window.debugAPI.getFallbackConfig() - Get fallback configuration'
      ] : [])
    ]
  }, 'info');
}
